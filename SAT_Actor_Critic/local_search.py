...
class SATLearner:
    ...
    def select_literal(self, f, list_literals):
        ...
        if sample == 1:
            ...
            self.last_10.insert(0, v)
            self.last_10 = self.last_10[:10]
        return literal, log_prob, log_prob_p, x, current_value
...
class WalkSATLN(SATLearner):
    ...
    def generate_episodes(self, list_f, critic_optimizer, scheduler_critic):
        ...
        for f in list_f:
            ...
            if sat and flips > 0 and not all(map(lambda x: x is None, log_probs)):
                loss, loss_p = self.reinforce_loss(log_probs, log_probs_p, temp_diff_list)
                losses.append(loss)
        ...
        if losses:
            ...
            if self.train_noise:
                ...
                losses_p = 0
        return all_flips, losses, losses_p, np.array(sats), np.mean(critic_losses)
    ...
    def train_epoch(self, optimizer, noise_optimizer, critic_optimizer, scheduler_critic, data):
        ...
        for list_f in batches:
            ...
            flips, loss, loss_p, sats, critic_losses = self.generate_episodes(list_f, critic_optimizer, scheduler_critic)
            acc = sats.mean()
            if acc > 0:
                optimizer.zero_grad()
                loss.backward()
            ...
            all_flips.append(flips)
            accuracy.append(acc)
            self.critic_t_loss.append(critic_losses)
        mean_loss = -1
        if mean_losses:
            mean_loss = np.mean(mean_losses)
        return flatten(all_flips), mean_loss, np.mean(accuracy), self.critic_t_loss