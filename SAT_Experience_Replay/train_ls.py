...
def train_policy(ls, optimizer, noise_optimizer, train_ds, val_ds, args, best_median_flips, model_files, start_time):
    best_epoch = 0
    ...
    for i in range(1, args.epochs + 1):
        print("epoch ", i)
        flips, loss, accuracy = ls.train_epoch(optimizer, noise_optimizer, train_ds, replay_buffer)
...
def main(args):
    ...
    ls = WalkSATLN(policy, noise_policy, args.train_noise, args.max_tries, args.max_flips, discount=args.discount, p=p, rb_epsilon=args.rb_epsilon, rb_samples=args.rb_samples)
    med_flips, mean_flips, accuracy = ls.evaluate(val_ds)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rb_epsilon', type=float, default=0.8)
    parser.add_argument('--rb_samples', type=int, default=10)
