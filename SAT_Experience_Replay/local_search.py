...
class SATLearner:
    def __init__(self, policy, noise_policy, train_noise=False, max_flips=10000, p=0.5, rb_epsilon=0.8, rb_samples=10):
        self.policy = policy
        self.model_np = ()
    ...
    def select_literal(self, f, list_literals):
        ...
        if sample == 1:
            ...
            literal, log_prob, x = self.reinforce_step(f, list_literals)
            v = abs(literal)
            self.age2[v] = self.flips
            self.last_10.insert(0, v)
            self.last_10 = self.last_10[:10]
        
        return literal, log_prob, log_prob_p, x
...
class WalkSATLN(SATLearner):
    def __init__(self, policy, noise_policy,  train_noise=False, max_tries=10, max_flips=10000, p=0.5, discount=0.5, rb_epsilon=0.8, rb_samples=10):
        super().__init__(policy, noise_policy, train_noise, max_flips, p, rb_epsilon, rb_samples)
        self.max_tries = max_tries
        self.rb_epsilon = rb_epsilon
    ...
    def select_variable_reinforce(self, x):
        ...
        v = dist.sample()
        return v, dist.log_prob(v), prob
    
    def reinforce_step(self, f, list_literals):
        x = self.stats_per_clause(f, list_literals)
        x = torch.from_numpy(x).float()
        index, log_prob, _ = self.select_variable_reinforce(x)
        literal = list_literals[index]
        return literal, log_prob, x
    ...
    def generate_episode_reinforce(self, f, replay_buffer):
        self.init_all(f)
        log_probs = []
        log_probs_p = []
        num_unsat_clauses = len(f.clauses)
        while self.flips < self.max_flips:
            ...
            indeces = np.random.choice(unsat_clause_indices, 1)
            list_literals = f.clauses[indeces[0]] 
            literal, log_prob, log_prob_p, x = self.select_literal(f, list_literals)
            self.update_stats(f, literal)
    ...
    def generate_episodes(self, list_f, replay_buffer):
        losses = []
        losses_p = []
        all_flips = []
        sats = []
        for f in list_f:
            sat, flips, log_probs, log_probs_p = self.generate_episode_reinforce(f, replay_buffer)
    ...
    def train_epoch(self, optimizer, noise_optimizer, data, replay_buffer):
        losses = []
        all_flips = []
        mean_losses = []
        accuracy = []
        ...
        for list_f in batches:
            ...
            if random.random() > self.rb_epsilon and len(replay_buffer) >= 10000:
                logs = []
                logs_p = []
                states = replay_buffer.sample(self.rb_samples)
                for x in states:
                    if x != None:
                        _, log_prob, log_prob_p = self.select_variable_reinforce(x)
                        logs.append(log_prob)
                        logs_p.append(log_prob_p)
                loss, loss_p = self.reinforce_loss(logs, logs_p)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            else:
                flips, loss, loss_p, sats = self.generate_episodes(list_f, replay_buffer)
                acc = sats.mean()
                if acc > 0:
                    optimizer.zero_grad()
                    loss.backward()
