...
class WalkSATLN(SATLearner):
    ...
    def reinforce_loss(self, log_probs, log_probs_p, values):
        T = len(log_probs)
        log_probs_filtered = []
        ...
        advantages = p_rewards - state_values.detach()
        loss = -torch.mean(p_rewards[torch.from_numpy(mask).to(log_probs.device)] * log_probs)
        loss_p = 0
        if self.train_noise:
            loss_p = -torch.mean(advantages * log_probs_p)
        return loss, loss_p
    ...
    def generate_episodes(self, list_f, critic_optimizer, scheduler_critic):
        ...
        for f in list_f:
            ...
            if sat and flips > 0 and not all(map(lambda x: x is None, log_probs)):
                ...
                loss, loss_p = self.reinforce_loss(log_probs, log_probs_p, values)
                losses.append(loss)
                losses_p.append(loss_p)
                R = 1
            else: R = 0
            sats.append(sat)  
            value_rewards = torch.full((values.size(0),), R, dtype=float)
            values = values.float()
            value_rewards = value_rewards.float()
            v_loss = F.mse_loss(values, value_rewards)
